...
import com.tcoded.hologramlib.hologram.ItemHologramLine;
...
public class NmsItemHologramLine extends ItemHologramLine {

    private final Display.ItemDisplay parent;

    public static NmsItemHologramLine create(PlaceholderHandler placeholderHandler) {
        // Accepts null values
        // noinspection DataFlowIssue
        Display.ItemDisplay parent = new Display.ItemDisplay(EntityType.ITEM_DISPLAY, null);
        return new NmsItemHologramLine(parent, placeholderHandler);
    }

    public NmsItemHologramLine(Display.ItemDisplay parent, PlaceholderHandler placeholderHandler) {
    ...
    @Override
    protected void sendMetaPacket(Collection<Player> players, @Nullable BiConsumer<Player, TextReplacementConfig.Builder> textParser) {
        ItemDisplayMeta meta = this.getMeta();

        // Set item
        ItemStack itemStack = meta.getItemStack();
        if (itemStack != null) {
            this.setNmsItemStack(itemStack);
        }
    ...
    @Override
    protected void sendTeleportPacket(Collection<Player> players, Location location) {
    ...
    private void setNmsItemStack(ItemStack itemStack) {
        net.minecraft.world.item.ItemStack nmsItem = CraftItemStack.asNMSCopy(itemStack);
        this.parent.setItemStack(nmsItem);
    }
    ...
    private void setNmsPositionRotationInterpolationDuration(int positionRotationInterpolationDuration) {
        this.parent.getEntityData().set(Display.ItemDisplay.DATA_POS_ROT_INTERPOLATION_DURATION_ID, positionRotationInterpolationDuration);
    }
